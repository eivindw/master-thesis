...
implemented in the same language. The boundary between external and
embedded DSLs is not necessarily a fixed one. Elements from both
categories can be mixed in a single DSL implementation, as shown by
Bravenboer and Visser in\cite{bra04}.
...
The goal platform of this thesis is JVM. One of the reasons for this
is that the popularity of the Java programming language has made the
JVM one of the most widespread multi-language platforms in the
...
limitiations set by the currently supported bytecode, as it was
...
to look into the performance of DSLs making using these kind of
features.
...
\begin{lstlisting}[language=java,caption=Groovy closure example.,label=lst:groovyclosure]
...
(\cite{hud96},\cite{hud98},\cite{kam98} and\cite{atk09}), making
...
to use, as it involves potensially many operations (depending on the
...
\vref{lst:scalaanonfunc}. Notice how the only thing differenciating
...
\begin{lstlisting}[caption=Scala anonymous function example.,label=lst:scalaanonfunc]
...
is a good technique found in Scala that enables combining two DSLs in
a smooth manner. The package templates mechanism research at the
University of Oslo seems like a good alternative to combine DSLs.
...
This chapter first provides a conclusion of the results found, and the
work performed. Secondly a section suggesting future work based on the
key findings is given.
...
\item \textbf{Scala} -- Scala is fast, due to its static typing, and
  ...
  a whole familiy of new features to the JVM.
...
tasks. However, the only area where it is clearly better than the
other languages seems in building custom control structures and
possibly removing existing ones (modifications to the host
language). Functional languages on the JVM seem to have had an uprise
in popularity the last years (my personal opinion based on observed
Internet activity), and Clojure provides the most ``pure'' functional
option.
...
languages offer, examplifying a super-concise syntax not matched by
...
% LocalWords:  createHistogram scalacompilerplugin namespaces cljundefcore
% LocalWords:  unmap functionalmetaprogramming pointcut
