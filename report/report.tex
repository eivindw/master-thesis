...
host language. This has some advantages in reducing the work needed to
implement the DSL (as a separate parser and interpreter does not have
to be implemented), and in integration with the host language and
other DSLs implemented in the same host language\cite{atk09}. The
boundary between external and embedded DSLs is not necessarily a fixed
one. Elements from both categories can be mixed in a single DSL
implementation, as shown in\cite{bra04}.

In this thesis I have focused solely on embedded DSLs running on the
Java Virtual Machine (JVM). As described in the book <<DSLs in
Action>> by Debasish Ghosh\cite{gho09}, several different programming
languages can be used to implement embedded DSLs on the JVM. In this
thesis I have identified some different requirements of embedded DSLs,
and then looked at various JVM programming languages (described in
section \vref{sec:programminglanguages}) in order to find out what
languages are best suited for the different requirements. The rest of
this chapter describes these requirements, with a discussion of why I
...
There are many situations where having the ability to customize a DSL
may be useful. There are also many different ways that the DSL can be
...
users should not have. Or in creating a general DSL that can be
tailored for each user according the the needs of that particular
user. If you only need feature X, and the general DSL includes
features X, Y and Z, then a specialized version only including feature
X could be created.
...
possible to tailor new DSLs including one the sub-domains needed. Two
users working on separate parts of the domain could have tailored DSLs
with some overlapping functionality where applicable.
...
parallel processing and concurrency?  There are many interesting
questions to be answered regarding performance.
...
pointer to the chapter that covers each set of requirements:

\begin{itemize}
\item \textbf{Syntax and Semantics} -- What capabilities does the
  different languages provide in building concise syntax and
  semantics? Chapter \ref{chp:syntax} <<Syntax and Semantics>> looks
  at different host language features, and how those features can be
  used to create embedded DSLs with a concise and understandable
  syntax.
\item \textbf{Customizing the DSL} -- How can a DSL be tailored for a
  ...
  handles the design of customizable DSLs.
\item \textbf{Changing the Host Language} -- How can unwanted features
  ...
  describes looks at how the various host languages can be changed.
