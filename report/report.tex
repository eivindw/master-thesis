...
\item Chapter \ref{chp:composition} <<Composition>> handles composition
  of DSLs.
\item Chapter \ref{chp:advancedtechniques} <<Advanced Techniques>> describes
  advanced DSL techniques like pruning (removing features from) and
  extending the host language.
\item Chapter \ref{chp:dsluse} <<Using the DSL>> handles different ways
...
\begin{lstlisting}[caption=Scala charting DSL -- create a basic pie chart,label=lst:chartdsl]
...
TODO: describe chess DSL. Drop example?
...
embedded DSL in Java. The example shown in listing \vref{lst:jmock} is
...
like the sorting shown in the examples above. In my image processing DSL
...
\begin{lstlisting}[caption=Scala word statistics example]
...
\begin{lstlisting}[caption=Scala example making functions look like built-in keywords]
