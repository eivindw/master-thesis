...
\item Write example -- The major part of the thesis work is of course
  related to the experiments conducted. The first three steps are
  important to make sure the experiments are based on sound evidence
  and former knowledge.
\item Measure -- Measure results, if applicable.
\item Conclude -- Make a conclusion based on the results found.
...
requirements. Note also that the method is used in cycles, after
evaluation of the results new or modified requirements can be taken
into consideration. This way of working is very good for a master
thesis of a set time/size, as the method is simply run over and over
again as many times as there is room for.
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[node distance = 2cm, auto]
    % Place nodes
    \node [cloud, fill=blue!10] (requirement) {1. Identify requirement};
    \node [cloud, fill=blue!20, right of=requirement, above of=requirement] (theory) {2. Search theory};
    \node [cloud, fill=blue!60, right of=requirement, below of=requirement] (evaluate) {6. Evaluate};
    \node [cloud, fill=blue!30, right of=theory] (example) {3. Write example};
    \node [cloud, fill=blue!50, right of=evaluate] (conclude) {5. Conclude};
    \node [cloud, fill=blue!40, right of=example, below of=example] (measure) {4. Measure};
    % Draw edges
...
\item \textbf{Isolation/uniqueness} -- How can unwanted features of
  ...
  be extended to provide unique functionality?
\item \textbf{Use} -- How can the DSL be used? What is the level of
  tool support provided for the DSL? How are errors and exceptional
  situations handled in the different languages?
\item \textbf{Performance} -- How does the different languages
  facilitate building DSLs with high requirement regarding
...
chapter, concluding the results found related to the requirement
...
development of various types of domain-specific languages. Some
...
studied are described and categorized in the next section.
...
TODO: describe integration between Groovy and Java.
...
was implemented, with regards to syntax and class/object composition.
...
TODO: functional example, using monads?
...
TODO: ref. monads described in sec 3.1 (also todo). Ref to Hudak monad paper.
...
TODO: dynamic language example for comparison.
...
What makes this possible is implicit conversions. For the
...
TODO: clojure example data structure. Same as Scala example with person?
...
TODO: say something about DSL use?
...
\begin{lstlisting}[language=Ruby,caption=Ruby on Rails \texttt{ActiveRecord} example.,label=lst:rubymethodmissing]
class ActiveRecord
  # ...
...
\vref{fig:iphistogram} shows the image used together with the
histogram produced.
...
\begin{figure}[h]
  \subfigure[Image]{\includegraphics[width=0.49\textwidth]{images/numbers}}
  \hfill
  \subfigure[Histogram]{\includegraphics[width=0.49\textwidth]{images/numbers_hist}}
  \caption{An image together with the generated histogram, showing the
    distribution of gray levels in the image.\label{fig:iphistogram}}
\end{figure}
