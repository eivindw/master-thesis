...
JVM with the help of the JRuby interpreter.
...
respect to embedding DSLs on the JVM.
...
them given in different programming languages. Chapter
\vref{sec:dslsyntax} looks at techniques for creating concise and
``fluent'' syntax, chapter \vref{sec:dslcomposition} handles
composition of DSLs and chapter \vref{sec:dsladvanced} describes
advanced DSL techniques like pruning and extending the host
language. Chapter \vref{sec:dsluse} handles different ways to utilize
and run the DSLs, and chapter \vref{sec:dslperformance} discusses
various aspects regarding DSL performance. Chapter \vref{sec:examples}
describes the example DSLs created in this thesis, while chapter
\vref{sec:summary} provides a summary containing the conclusion as
well as some suggestions to further work.
...
in Java. It allows programmers writing unit tests to specify with an
internal DSL syntax how they want to mock dependencies to the code
they are testing. Example written in Java (DSL syntax mostly found in
the expectations declarations):
...
Run as a script. All except Java?

\section{Interactive Console -- Interpreter}

Scripting console with DSL preloaded. All except Java?

\section{Embedded Interpreter}

DSL preloaded interpreter (as previous section) embedded in another
application. Supported in Scala.

\section{Tool Support}

Comparison of tools support for the various languages?

\chapter{DSL Performance}
\label{sec:dslperformance}
...
OpenCL on Java/Scala. Cuda or similar?
