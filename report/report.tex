...
\begin{itemize}
\item Chapter \ref{chp:examples} <<Programming Languages and
  Examples>> describes the programming languages used and the example
  DSLs created in this thesis.
\item Chapter \ref{chp:syntax} <<Syntax>> looks at techniques for
  creating concise and ``fluent'' syntax.
\item Chapter \ref{chp:composition} <<Composition>> handles
  composition of DSLs.
\item Chapter \ref{chp:advancedtechniques} <<Advanced Techniques>>
  describes advanced DSL techniques like pruning (removing features
  from) and extending the host language.
\item Chapter \ref{chp:dsluse} <<Exploring Ways to Use Embedded DSLs>>
  handles different ways to utilize and run the DSLs.
\item Chapter \ref{chp:performance} <<Performance>> discusses various
  aspects regarding DSL performance.
\item Chapter \ref{chp:summary} <<Summary>> provides a summary
...
object acting as a factory for building datasets. The source code is
given in listing \vref{lst:statsdsl}.

\begin{lstlisting}[caption=Minimal statistics DSL implemented in Scala.,label=lst:statsdsl]
...
Using the statistics DSL is typically done by creating a dataset from
...
value of a standard 6-sided dice (numbers 1-6) is given below:
...
\begin{lstlisting}[float,caption=Scala charting DSL -- create a basic pie chart.,label=lst:chartdsl]
...
\begin{lstlisting}[float,language=ruby,caption=Example using the Ruby on Rails \texttt{ActiveRecord} database querying classes.,label=lst:activerecordex]
...
\begin{lstlisting}[caption=ScalaTest example using implicit type conversions.,label=lst:scalatestimplicitconversion]
...
"A Stack" should "pop values in last-in-first-out order" in {
...
  stack.pop() should equal (2)
...
\begin{lstlisting}[caption=ScalaTest implicit type conversion definitions.,label=lst:scalatestimplicitconversiondef]
...
\begin{lstlisting}[caption=Scala image processing functions implemented using anonymous functions]
...
\begin{lstlisting}[float,caption=Ruby on Rails example showing dynamic metaprogramming.,label=lst:railsmeta,language=Ruby]
...
\begin{lstlisting}[float,caption=Ruby dynamic metaprogramming example.,label=lst:rubydynmeta,language=Ruby]
class ActiveRecord
  ...
...
\begin{lstlisting}[language=Ruby,caption=Ruby example showing ``monkey patching'' on open classes.]
class Integer
  def doubleIt
    self * 2
  end
...
Multimethod example shown in \vref{lst:cljmm}.
...
\begin{lstlisting}[caption=Scala example using implicit conversion to combine DSL for statistics with DSL for image processing.]
...
\begin{lstlisting}[float,caption=Scala example making functions look like built-in keywords.,label=lst:scalabuiltinkeywords]
...
TODO: Clojure macro examples. Clojure \texttt{unless} implementation
as a macro shown in listing \vref{lst:cljunless}.
...
\begin{lstlisting}[caption=PreDef.scala file with image processing imports.]
import simage.io.SImageIO._
import simage.structs._
import simage.structs.StrElType._
\end{lstlisting}
...
\begin{lstlisting}[language=,caption=Scala REPL execution with preloaded import statements.]
...
image processing example shown in chapter \vref{chp:examples} it would
...
impact on the performance on the JVM is wether the language is
...
using regular static types instead. However, a thourough knowledge of
...
\item Groovy -- the slowest of the all.
\end{enumerate*}
...
\begin{lstlisting}[float,caption=Matrix implementation based on List of Lists.,label=lst:listoflist]
...
\begin{lstlisting}[float,caption=Matrix implementation based on 2D Array.,label=lst:2darray]
...
\begin{lstlisting}[float,caption=Implementing matrix neighbour average using a general structuring-element operation.,label=lst:strelavg]
class Matrix ... {
...
\begin{lstlisting}[float,caption=Implementation of a structuring element operation on a matrix using \texttt{for comprehensions}.,label=lst:forcomp]
def seOp(se: Matrix, op: (Seq[Int]) => Int) = {
  val w = se.nCols / 2
  val h = se.nRows / 2
...
\begin{lstlisting}[float,caption=Implementation of a structuring element operation on a matrix using \texttt{while loops}.,label=lst:whileloop]
def seOp(se: Matrix, op: (Seq[Int]) => Int) = {
  val w = se.nCols / 2
  val h = se.nRows / 2
...
\begin{lstlisting}[float,caption=A generic \texttt{Executor} based on the Scala Actors API.,label=lst:actorexecutor]
import actors.Actor
...
\begin{lstlisting}[float,caption=An example using \texttt{Futures} to execute a function in a separate thread.,label=lst:futureex]
...
\begin{lstlisting}[float,caption=A general parallel mechanism based on futures.,label=lst:futureipex]
import actors.Futures._
...
programming. If you know that the same state can not be overriden by
...
functions implement the \texttt{java.util.concurrent.Callable}
interface, from the concurrent classes added to the JDK from version
1.5. This allows all Clojure functions being able to run directly in
separate Java thread, or using executors or other mechanisms from the
Java concurrent API. So spawning several threads to perform separate
tasks is extremely easy. An example is shown in the following listing,
where the function printing ``hello!'' is run in a separate thread on
the JVM:
...
TODO: create Clojure matrix example with multithreaded use.
...
compiled/intepreted with regards to DSL types and wanted
...
also are scathered throughout the report, it is important to gather
...
done in parallell with the dynamic type support described in the
previous section.
