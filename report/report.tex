...
\item Annotations -- Annotations were introduced to Java with version
  ...
  embedding DSLs.
\item Reflection
\item AOP eller lignende?
\end{itemize}

TODO: Example from Hibernate? To compare with the Rails examples?

\begin{lstlisting}[language=java]
@Named @SessionScoped
public class SpecialService() {

  @Random @Inject int randomNumber;
...
Scala has no special mechanism for metaprogramming yet.
...
relationships between the classes \texttt{Team} and
\texttt{Player}. The methods \texttt{has\_many} and \texttt{belongs\_to}
are typically implemented in a manner similar to the Groovy examples
above, adding methods \texttt{team} and \texttt{players} with database
extraction logic to the classes in use:
...
\begin{itemize}
\item Lisp makroer + innebygget metadata-støtte
\item Egen syntax -- \texttt{meta} og \texttt{with-meta}
\item Vanskelig å skjønne uten FP/Lisp bakgrunn
\end{itemize}
...
functionality, using the image analysis DSL from chapter
\vref{sec:examples} as example (loading an image and saving a blurred
copy to a new file):
