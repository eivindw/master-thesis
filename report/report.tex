...
(some examples are\cite{hud96},\cite{hud98} and\cite{atk09}), making
...
\subsection{Programming Languages and Tasks}
...
will get added to the Java programming language in a future release. I
...
Table \vref{tab:taskslangs} gives an overview of some characteristics
of tasks, and what programming language a DSL should be implemented
in. An \texttt{x} in the table means good match, while an \texttt{(x)}
means partially good match and a \texttt{-} means not recommended.
...
\begin{table}[ht]
  ...
  \begin{tabular*}{1\textwidth}{l c c c c c l}
    ...
    Efficient bytecode/high performance & x & x & - & - & - & Image Processing \\
    Good concurrency abstractions & (x) & x & - & - & x & Image Processing \\
    Combining DSLs & - & x & - & x & x & Image Analysis \\
    Custom control structures & - & (x) & - & - & x & Modify host language \\
    Good error checking & x & x & - & - & - & Space Ship DSL \\
    Interactive interpreter & - & x & x & x & x & All \\
