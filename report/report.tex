...
\begin{itemize}
\item \textbf{Cross-platform distribution} -- Code written in any of
  the languages can be run on any platform supporting JVM 1.5 or
  newer.
...
\item \textbf{Generics implemented with erasure} -- This was a design
  ...
  information at compiletime. For example for Scala, a language which
...
Groovy is a dynamically typed and interpreted (or compiled) language
supporting the object-oriented paradigm. It is tightly integrated with
Java and can leverage existing Java libraries directly. An example of
a simple object-oriented ``Hello World!'' program is shown in listing
...
example\cite{hud96} and \cite{hud98}), making Clojure an interesting
language with respect to embedding DSLs on the JVM. Below is an
example of the ``hello world'' function in Clojure, as well as an
example implementation of the Fibonacci function (a more common
...
Clojure has big collection of features not found in any of the other
languages, many which will be explained in more detail where
...
of traits in the previous section (TODO: referanse til
polymorfisme). Clojure, being a functional language, supports a
different mechanism to obtain similar effects. This is called
\textit{multimethods}\cite{hal09}, and is basically a kind of dynamic
dispatch where you specify a function to perform the dispatch rather
than only the type as is done in polymorphism. Instead of the type
itself providing some implementation of an interface, the whole
dispatch is done outside using the type as a parameter. A very basic
example is shown in listing \vref{lst:cljmm}. The \texttt{my-print}
function uses the \texttt{class} function, which returns the class of
an object, to dispatch on the argument. So the method can be called on
objects of type \texttt{String} or \texttt{Number} (supertype for
numeric types in Java). This example does basically the same as
object-oriented languages do with polymorphism, it dispatches based on
...
All in all the multimethods mechanism can be seen as an extremely
powerful way to customize the way functions are dynamically dispatched
in a DSL. It is a complicated mechanism that probably should be used
with care, but the possibilities it provides are not found in any of
the other languages studied in this thesis.
...
might be better. \cite{ode08} describes the \textit{loan pattern},
where a control-abstraction function opens a resource and ``loans'' it
to a function before it closes the resource. The exception handling
would then be done in this function providing a control-abstraction,
and not by the calling function. An example is shown in listing
\vref{lst:scalaloanpattern}, using the \texttt{withPrintWriter}
abstraction to write a date to a file ("date.txt") without handling
exceptions. This would be valuable in a DSL setting where we might
want to hide ``complicated'' exception handling from the user.
...
difference might be important for some kinds of DSLs.

The major difference between compiling or not is of course related to
the typing dicipline of the language. The statically typed languages
...
JRuby time: 1071ms
Scala time: 18ms
Clojure "Elapsed time: 185.542 msecs"
\end{lstlisting}
...
are much slower on the JVM. This might change, as mentioned in the
...
the functional programming features of Scala might come at the price
of efficiency.
...
possible, as stated in\cite{ode08}: \textit{The Scala compiler uses
  ...
  in the compiled code.} However, it is not allways obvious when this
...
resources. With multithreaded programming there are many problems that
can occur, like deadlocks/livelocks (two threads blocking eachother
...
memory without synchronizing with eachother).

The main mechanisms provided in Java can be summerized in the
following list:
...
a small DSL for enabling parallelization in Scala.
...
datastructures. In the example two 5-dimensional arrays are created
and added together.
...
thouroughly studied was the traits in Scala, which were used to build
the DSL for image processing.
...
In Scala, compiler plugins can be written to provide new or remove
...
The dynamic languages support mofifications of classes at
...
written in Clojure itself, and can be redefined by the user if
...
DSL that was written in Scala, but almost entirely based on an
...
that the JVM was not written with dynamic languages in mind, but might
