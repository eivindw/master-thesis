...
\begin{figure}[t]
  \subfigure[Clojure]{\lstinputlisting[language=lisp,linewidth=0.48\textwidth]{../examples/clj-java/ex.clj}}
  \hspace{37pt}
  \subfigure[Java]{\lstinputlisting[language=java,linewidth=0.48\textwidth]{../examples/clj-java/ex.java}}
...
directly execute statments is extremely useful when you need to test a
...
\vref{fig:intellijss}. A thourough comparison between the various
...
\begin{figure}
  ...
  \caption{Screenshot from IntelliJ IDEA, an Integrated Development
...
concise coding-style. Code looks very clean without information about
types included. Another extremely powerful mechanism only available in
the dynamic languages is the dynamic metaprogramming described in
section \vref{sec:dynamicmetaprogramming}. Code that dynamically
generates new code provides a basis for popular DSL environments like
...
effective with regards to the domain-spesific language angle. For me
personally it has been an extremly interesting year, with a massive
